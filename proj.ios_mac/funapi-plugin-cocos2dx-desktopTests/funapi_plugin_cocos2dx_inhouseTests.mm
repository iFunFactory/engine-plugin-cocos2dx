...
- (void)testFunapiWebsocket {
  std::string server_ip = g_server_ip;
  int server_port = 18012;
...
- (void)testCompressionZstdDic {
  ...
  XCTAssert(in_string.compare(out_string) == 0);
}
...
- (void)testCompressionPerformance {
  ...
  {
    auto compression = std::make_shared<fun::FunapiCompression>();
    compression->SetCompressionType(fun::CompressionType::kDeflate);
