...
- (void)testSessionMultithread {
  ...
  [self, &send_message, kMaxCount, &complete_mutex, &v_completed]
  (const int index,
   ...
   const bool use_session_reliability)
  {
    ...
    session->AddProtobufRecvCallback
    ([index, &send_message, &is_working, &is_ok]
     (const std::shared_ptr<fun::FunapiSession> &s,
      const fun::TransportProtocol protocol,
      const FunMessage &message)
    {
...
- (void)testSessionMultithread_UpdateAll {
  const int kMaxThread = 2;
  const int kMaxCount = 50;
  std::vector<std::thread> temp_thread(kMaxThread);
  std::vector<bool> v_completed(kMaxThread);
...
- (void)testNoneEventReconnectSend10Times {
  std::string send_string = "Json Echo Message";
  std::string server_ip = g_server_ip;
...
- (void)testHttpReconnectSend10Times {
  std::string send_string = "Json Echo Message";
  std::string server_ip = g_server_ip;
...
- (void)testUdpReconnectSend10Times {
  std::string send_string = "Json Echo Message";
  std::string server_ip = g_server_ip;
  ...
  session->Connect(fun::TransportProtocol::kUdp);
...
- (void)testFunapiErrorCode {
  auto code = fun::FunapiError::ErrorCode::kRedirectConnectAuthFailed;
  auto error = fun::FunapiError::Create(fun::FunapiError::ErrorType::kRedirect, code);
  fun::FunapiError::ErrorCode ret_code = static_cast<fun::FunapiError::ErrorCode>(error->GetErrorCode());
  XCTAssert(ret_code == code);
}

- (void)testConnectConnectClose {
  std::string send_string = "Json Echo Message";
  std::string server_ip = g_server_ip;
  ...
  session->Connect(fun::TransportProtocol::kTcp); // test code
...
- (void)testConnectCloseClose {
  ...
  session->Close(fun::TransportProtocol::kTcp);

  session->Close(); // test code
